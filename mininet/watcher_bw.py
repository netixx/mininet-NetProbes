...
def _setMatches(watcherSets, graphSets, watcherPoint):
    ...
    representatives = sorted([(v['representative']['bw'], k) for k, v in watcherSets.iteritems()], reverse = True)
...
def _makeLinkResults(watcher, topoGraph, out, ipToName, grRoot = GRAPH_ROOT):
    reps = sorted([(v['representative']['bw'], k) for k, v in watcher['sets'].iteritems()], reverse = True)
...
class SetPlotter(Plotter):
    ...
    def barRandMeasure(self, variables = None, parameters = None, grouping = None, decoration = None):
        ...
        plot = False
        width = 0.7 * ((1.0 / len(parameters)) if len(parameters) > 0 else 1)
        # width = basewidth
        offset = 0
        scaleDict = {}
