...
_MON_COLLECT_TPL = "sudo iptables -n -L {direction} -v"
...
_CHAIN_TPL = r"^Chain (%s)" % r"|".join(_directions)
    ...
    for line in output.splitlines():
        ...
        if m is not None:
            if counter is not None:
                ...
                counter.counters[direction]['bytes'] += nbytes
                counter.counters[direction]['packets'] += int(m.group(1))
            out += "{:<12} {:<10} {:<10}\n".format(m.group(3).replace(r' dpt', ''), m.group(1), m.group(2))
...
class Counter(object):
    ...
    def __init__(self):
        ...
        for d in _directions:
            self.counters[d] = {'bytes' : 0,
                                'packets' : 0}

    def printAll(self):
        o = "Packets/Bytes summary :\n"
        for d in _directions:
            o += "Direction {:<10}: packets : {packets:<10}, bytes : {bytes:<10}\n".format(d, **self.counters[d])
        return o
